//! Parallel Classification Benchmark (Diabetes/Pima Indians)
//!
//! This benchmark uses ParallelTrainer for data-parallel batch processing.
//! Compare with bench_classification_diabetes_sgd.rs to see parallelization benefits.
//!
//! Run with: cargo run --release --example bench_classification_diabetes_parallel
...
use rusty_axon::loss::CrossEntropy;
...
// Convert label to one-hot encoding (f64 version for batch)
...
// Convert label to one-hot encoding (Node version for loss computation)
...
// Compute loss on dataset using framework's CrossEntropy
fn compute_loss(model: &Mlp, x: &[Vec<f64>], y: &[usize], loss_fn: &CrossEntropy) -> f64 {
    let total_loss: f64 = x.iter()
        ...
        .map(|(x_i, &y_i)| {
            ...
            let logits = model.forward(&inputs);
            let targets = one_hot_node(y_i, 2);
...
fn main() {
    println!("=== Rusty-Axon: PARALLEL Classification Benchmark (Diabetes/Pima Indians) ===\n");
    ...
    // Setup loss function using framework's CrossEntropy
    ...
    let output_path = "python-tests/rusty-axon-multi-thread/classification-diabetes/rust_classification_metrics_parallel.csv";
    
    // Create directory if it doesn't exist
    std::fs::create_dir_all("python-tests/rusty-axon-multi-thread/classification-diabetes").ok();
    ...
    for epoch in 1..=epochs {
        ...
        for batch_start in (0..x_train.len()).step_by(batch_size) {
            ...
            let batch: Vec<(Vec<f64>, Vec<f64>)> = batch_indices
                .iter()
                .map(|&idx| {
                    let inputs = x_train[idx].clone();
                    let targets = one_hot(y_train[idx], 2);
                    (inputs, targets)
                })
                .collect();
            
            // Train batch in parallel using ParallelTrainer
            trainer.train_batch(&mut mlp, &batch, &loss_fn);
        }
