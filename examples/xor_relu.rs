...
fn main() {
    ...
    let mlp = Mlp::new(&[2, 4, 1], &[Activations::ReLU, Activations::Sigmoid]);
    ...
    let edge_config = NetworkVisualizationConfig::with_colors(
        ...
        "royalblue",
        "mistyrose",
        "coral", // Different colors to distinguish from Tanh version
        "honeydew",
        "mediumseagreen",
    )
    .with_edges("darkslategray", 1.5);
    mlp.visualize_network_with_config("network_xor_relu", "png", &edge_config)
        .unwrap();
