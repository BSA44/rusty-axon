...
//! (forced naive, used by the `naive-matmul` feature flag and by
//! `--no-default-features --features train` builds that drop the
//! `matrixmultiply` dep).
//!
//! The two kernels are functionally identical to within rounding.  Phase 4's
//! kernel-agreement test (`engine::tests::test_kernel_agreement_64x64`)
//! verifies this when both implementations are compiled — i.e. when the
//! `matrixmultiply` feature is on.
