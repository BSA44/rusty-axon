...
impl CrossEntropy {
    ...
    fn softmax(&self, logits: &[Node]) -> Vec<Node> {
        ...
        let exp_logits: Vec<Node> = logits.iter()
        .map(|l| (l.clone() - max).exp())
        .collect();

        let sum_exps = exp_logits.iter()
        .cloned()
        .reduce(|a, b| a + b)
        .unwrap();
    ...
    fn smooth_targets(&self, targets: &[Node]) -> Vec<Node>
    {
        ...
        targets.iter()
        .map(|t| (1.0 - self.label_smoothing)*t.clone() + self.label_smoothing/(num_of_classes as f64))
        .collect() 
    }
...
impl Loss for CrossEntropy {
    fn forward(&self, logits: &[Node], targets: &[Node]) -> Node {
        assert_eq!(logits.len(), targets.len(), "Predictions and targets must have the same length");
        ...
        for (probability, target) in probabilities.iter().zip(smooth_targets.iter()) {
            loss = loss + (-target.clone() * (probability.clone() + 1e-10).log(std::f64::consts::E)); //adding small const to avoid erros
        }
